...
export class UserProvider {
  ...
  addDefaultUserAccount(){
    return this.storage.get(ITEMS_KEY).then((userAccounts: 
      ...
      UserAccount[]) => {
        ...
        else{console.log('UserAccount available!');
  ...
  addUserAccount(user: UserAccount):Promise<any>{
    return this.storage.get(ITEMS_KEY).then((userAccounts: 
      UserAccount[]) => {
        if(userAccounts){userAccounts.push(user);
          console.log('Oh Yes!');
          return this.storage.set(ITEMS_KEY, userAccounts);
        }
  ...
  //Get ALl UserAccount
  getAllUserAccount(){return this.storage.get(ITEMS_KEY);
  }
      ...
      updateUserAccount(updatedUser: UserAccount){
        return this.storage.get(ITEMS_KEY).then((userAccounts: UserAccount[]) => {
          if(!userAccounts || userAccounts.length == 0){
            ...
            for(let user of userAccounts){
              ...
              else{newUserAccounts.push(user);
              }
            }
        ...
        Promise<UserAccount>{
          return this.storage.get(ITEMS_KEY).then((userAccounts: UserAccount[]) => {
            ...
            for(let user of userAccounts){
              if(user.id !== search_id){toKeep.push(user);
              }
            }
